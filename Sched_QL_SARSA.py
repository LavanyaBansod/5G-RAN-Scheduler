...
# Suppress specific warnings related to gym
...
def Q_learning(env, episodes, learn_rate, epsilon, gamma, reduce_eps):
    ...
    for n in range(episodes):
        ...
        if epsilon > 0.01:
            ...
            epsilon = 0.01  # Ensure epsilon doesn't go below a threshold
...
def SARSA(env, episodes, learn_rate, epsilon, gamma, reduce_eps):
    ...
    for n in range(episodes):
        ...
        # Add a lower bound to epsilon decay to avoid it getting too small
        if epsilon > 0.01:
            epsilon -= reduce_eps
        else:
            epsilon = 0.01
        ...
        while not done:
            ...
            Q[state, action] = Q[state, action] + learn_rate * (rew + gamma * np.max(Q[next_state]) - Q[state, action])
